from django.shortcuts import render, redirect, HttpResponse # -> precisa importar o HttpResponde
...
def investimentos(request):
    dados = {
        'dados': Investimento.objects.all() #-> vá até banco de dados, encontre todos os registro e retorne todos eles
    }
...
def excluir(request,id_investimento):
    investimento = Investimento.objects.get(pk=id_investimento)
    if request.method == 'POST': # aqui o método é POST porque estamos enviando dados a partir de uma página, exclusão é como um método POST
        investimento.delete()
        return redirect('investimentos')
